<script lang="ts">
	...
	let { offer, onSuccessClose }: Props = $props();
...
<div class="scrim" onclick={handleClose}></div>
	...
	{#key $currentLocale}
		{#if !isSubmitted}
			<div class="form-wrap">
				<h2>{m.apply_title()}</h2>
				{#if offer}
					<p class="for">{m.apply_subtitle()} <b>{offer.lang[$currentLocale]?.jobType || offer.jobType} · {offer.location}, {offer.city}</b></p>
				{/if}
		...
		{:else}
			<div class="success">
				...
				<button class="btn-send" style="background:var(--surface-2);color:var(--ink)" onclick={handleSuccessClose}> {m.apply_success_back()} </button>
			</div>
		{/if}
	{/key}
</div>
